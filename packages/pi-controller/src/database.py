...
async def record_run_end(run_id: str, end_time: datetime, duration_seconds: int) -> None:
    async with aiosqlite.connect(_db_path) as db:
        await db.execute(
            """UPDATE watering_history
               SET end_time=?, duration_seconds=?, completed=1
               WHERE run_id=?""",
            (end_time.isoformat(), duration_seconds, run_id),
        )
        await db.commit()
